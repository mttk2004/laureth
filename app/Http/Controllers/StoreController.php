...
class StoreController extends Controller
{
     ...
     */
    public function getStoreStaff(Store $store)
    {
        // Kiểm tra quyền truy cập
        $currentUser = Auth::user();

        // Nếu người dùng không phải DM và không thuộc cửa hàng này, từ chối truy cập
        if ($currentUser->position !== 'DM' && $currentUser->store_id !== $store->id) {
            return response()->json(['message' => 'Không có quyền truy cập'], 403);
        }

        // Lấy danh sách nhân viên của cửa hàng
        $staff = \App\Models\User::where('store_id', $store->id)
